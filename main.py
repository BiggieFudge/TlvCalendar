...
response = requests.get(soup.find('li',class_="my-calendar-next").a['href'],headers)
...
for event in event_cells:
    dateStr = event.find('span', class_='mc-date').get_text(strip=True)
    ...
    formatted_date = f"{month_number}/{int(day):02d}/{year}"
...
file_name += "-" + events_database.iloc[0]['Start date'][0:2] +  "-"  +events_database.iloc[-1]['Start date'][0:2]
